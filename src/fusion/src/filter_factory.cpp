...
namespace Filter{
    ...
    std::unique_ptr<Fusion> FilterFactory::createFilter(FilterType type_ ,  std::shared_ptr<MotionModel> motion_model_ , std::shared_ptr<StateSpace> states_)
    {
       ...
       if(it_ != filters_.end())
       {
            ...
            filter_->setMotionModel(motion_model_);
            return filter_;
       }
       return nullptr;
    }
